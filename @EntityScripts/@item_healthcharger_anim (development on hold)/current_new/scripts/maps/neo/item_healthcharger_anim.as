/*
* |=============================================================================|
* | I T E M _ H E A L T H C H A R G E R   -   MAP SCRIPT    [#include version]  |
* | Author:  Neo (Discord: NEO) Version: BETA (with bugs) / © 2019              |
...
* |=============================================================================|
* | This entity map script enables an alternative health charger station.       |
* |=============================================================================|
* | Map script install instructions:                                            |
* |-----------------------------------------------------------------------------|
* | 1. Extract the map script 'scripts/maps/neo/item_healthcharger.as'          |
...
class item_healthcharger_pillar : ScriptBaseAnimating
{
	uint  m_uCrtlLvl		= 0;
	float m_flCtrlLvlMin	= -11.0f;
	float m_flCtrlLvlMax	= 0.0f;
	string m_szModelName	= "models/neo/health_charger_pillar.mdl";
...
class item_healthcharger_anim : ScriptBaseAnimating
{
	float 	m_flHealthChargerRechargeTime = 60.0f;
	...
	float 	m_flSoundTime;
	bool 	m_bIsUsed;
	bool 	m_bIsFrontAngle;
	RechargeState2 CurrentState;
	item_healthcharger_pillar@ pBottle;
	string 	m_szModelName    = "models/neo/health_charger_body.mdl";
	string 	m_szMedShotSnd   = "items/medshot4.wav";
	string 	m_szMedsHotNoSnd = "items/medshotno1.wav";
	string 	m_szMedChargeSnd = "items/medcharge4.wav";
	...
	void Spawn()
	{
		...
		// Spawn bottle entity
		CBaseEntity@ pBottleEnt = g_EntityFuncs.CreateEntity( "item_healthcharger_pillar", null,  false );
		@pBottle = cast<item_healthcharger_pillar@>(CastToScriptClass(@pBottleEnt));
		pBottle.Spawn();
...
void RegisterItemHealthCHargerAnimEntity()
{
    g_CustomEntityFuncs.RegisterCustomEntity( "item_healthcharger_pillar", "item_healthcharger_pillar"  );
    g_CustomEntityFuncs.RegisterCustomEntity( "item_healthcharger_anim", "item_healthcharger" );
}
